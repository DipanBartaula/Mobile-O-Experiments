...
class mobileoTrainer(Trainer):
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            #  NEW: Identify layer_weights parameters (similar to mm_projector)
            layer_weights_parameters = [name for name, _ in opt_model.named_parameters() if "layer_weights" in name]
    
            #  NEW: Check if we should use special LR for layer_weights
            layer_weights_lr = getattr(self.args, 'layer_weights_lr', None)
            if layer_weights_lr is None:
                # Default: 100× higher than base LR
                layer_weights_lr = self.args.learning_rate * 10    
                ...
                optimizer_grouped_parameters = [
                    {
                        "params": [
                            ...
                            if (n in decay_parameters
                                ...
                                and n not in layer_weights_parameters  #  Exclude layer_weights
                                and p.requires_grad)
                        ],
                   ...
                   
                    {
                        "params": [
                            p for n, p in opt_model.named_parameters()
                            if (n in layer_weights_parameters and p.requires_grad)
                        ],
                        "weight_decay": 0.0,  # No weight decay for layer_weights
                        "lr": layer_weights_lr,
                    },
                ]
            else:
            ...
            for i, group in enumerate(optimizer_grouped_parameters):
                ...
                if num_params > 0:
                    ...
                    print(f"Group {i}: {num_params} params, LR={lr:.2e}, WD={wd:.4f}")
    
                    # Check if this group contains layer_weights
                    sample_param = group['params'][0]
                    for name, param in opt_model.named_parameters():
                        if param is sample_param:
                            if 'layer_weights' in name:
                                print(f"  ✓ Contains layer_weights: {name}")
                            elif 'mm_projector' in name:
                                print(f"  ✓ Contains mm_projector")
                            break
            print("=" * 70 + "\n")
